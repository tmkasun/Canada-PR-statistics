...
import { sendBCPNPmail, sendEEEmail, sendOINPEmail } from "./libs/email";
import { getDrawsFromBCPNP, getLatestBCPNPFromDB } from "./bcpnp";
import { IBCPNPDraw } from "./bcpnp/utils";

const IRCC_DATA_JSON =
  "https://www.canada.ca/content/dam/ircc/documents/json/ee_rounds_123_en.json";

const getLatestDrawFromIRCC = async () => {
  logger.info("Fetching IRCC data!");
  const res = await fetch(IRCC_DATA_JSON);
...
const getLatestDrawFromDB = async (
  ...
  let lastKnownDraw = await db.findOne<{ lastDraw: number; _id: any }>({
    program: "ee",
  });
  if (!lastKnownDraw) {
    const done = await db.insertOne({
      program: "ee",
      lastDraw: latestDrawNumber,
    } as any);
...
(async () => {
  ...
  try {
    ...
    const latestBCPNPFromDB = await getLatestBCPNPFromDB(
      mongoCollection,
      latestBCPNPFromWeb.date
    );
    if (latestBCPNPFromWeb.date > latestBCPNPFromDB.lastDraw) {
      logger.info(
        `Found a new BC PNP invitation round from web: ${latestBCPNPFromWeb}`
      );
      const done = await mongoCollection.updateOne(
        { _id: { $eq: latestBCPNPFromDB._id } },
        { $set: { lastDraw: latestBCPNPFromWeb.date } }
      );
      await sendBCPNPmail(
        latestBCPNPListFromWeb.filter(
          (draw: IBCPNPDraw) =>
            draw.date.getDate() === latestBCPNPFromWeb.date.getDate()
        )
      );
    } else {
    ...
    if (latestDrawNumberFromIRCC > latestDrawFromDB.lastDraw) {
      ...
      const done = await mongoCollection.updateOne(
        { _id: { $eq: latestDrawFromDB?._id } },
        { $set: { lastDraw: latestDrawNumberFromIRCC } }
      );
    ...
    if (latestOINPFromWeb.date > latestOINPFromDB.lastDraw) {
      ...
      const done = await mongoCollection.updateOne(
        { _id: { $eq: latestOINPFromDB._id } },
        { $set: { lastDraw: latestOINPFromWeb.date } }
      );
